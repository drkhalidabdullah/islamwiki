...
echo "=== TESTING ACHIEVEMENT ACE LOGIC ===\n\n";
...
foreach ($users as $user) {
    ...
    // Get user level
    $user_level = $achievements_extension->getUserLevel($user['id']);
    echo "  Total achievements: {$user_level['total_achievements']}\n";
    
    // Get Achievement Ace achievement
    $stmt = $pdo->prepare("SELECT * FROM achievements WHERE slug = 'achievement-ace'");
    $stmt->execute();
    $achievement = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if ($achievement) {
        echo "  Achievement Ace requirement: {$achievement['requirement_value']} achievements\n";

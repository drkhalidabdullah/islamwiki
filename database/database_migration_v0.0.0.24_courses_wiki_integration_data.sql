-- Courses Wiki Integration Data Migration
-- Version: 0.0.0.24
-- Description: Migrate existing course data to wiki system

-- 1. Create course categories as wiki categories (if not exists)
...
AND NOT EXISTS (
    ...
    WHERE wa.slug = CONCAT('Course:', c.slug) 
    AND wa.course_type = 'course'
);

-- 5. Migrate course lessons to wiki articles (only if not already migrated)
INSERT INTO `wiki_articles` (
    `title`, `slug`, `content`, `excerpt`, `category_id`, `author_id`, `status`,
...
AND NOT EXISTS (
    SELECT 1 FROM wiki_articles wa2 
    WHERE wa2.slug = CONCAT('Course:', c.slug, '/', cl.slug) 
    AND wa2.course_type = 'lesson'
);

-- 6. Create wiki-based course progress tables (if not exist)
...
-- 7. Migrate user course progress to new wiki-based system (only if not already migrated)
...
AND NOT EXISTS (
    SELECT 1 FROM wiki_course_progress wcp 
    WHERE wcp.user_id = ucp.user_id 
    AND wcp.course_article_id = wa_course.id
);

-- 8. Migrate user course completions to new wiki-based system (only if not already migrated)
...
AND NOT EXISTS (
    SELECT 1 FROM wiki_course_completions wcc 
    WHERE wcc.user_id = ucc.user_id 
    AND wcc.course_article_id = wa_course.id
);

-- 9. Create a special "Courses" index page (if not exists)
